...
public class SuperSqlBuilder extends SqlBuilder {
     ...
     * @return sqlBuilder
     */
    public SqlBuilder insertBatch(String tableName, List<Map<String, Object>> entities, Dialect dialect) {
        Assert.notEmpty(entities, "批量插入时，数据不能为空");
        // 完成SQL
        return SqlBuilder.of(buildPsInsert(tableName, entities, dialect));
    }
     ...
     * @return 预编译的SQL
     */
    private StringBuilder buildPsInsert(String tableName, List<Map<String, Object>> entities, Dialect dialect) {
        // 对Oracle的特殊处理
        ...
        return new StringBuilder(DialectName.PHOENIX.match(dialect.dialectName()) ? "UPSERT INTO " : "INSERT INTO ")
                .append(tableName).append(" (").append(fieldsPart).append(") VALUES ")
